...
def create_point_cloud(color_raw, depth_raw, camera_intrinsic, dist_coeff):
    ...
    """
    # Convert Open3D image to numpy for OpenCV operations
    color_np = np.asarray(color_raw)
    ...
    color_undistorted_np = cv2.undistort(color_np, camera_matrix, dist_coeff)
    depth_undistorted_np = cv2.undistort(depth_np, camera_matrix, dist_coeff)
    
    # Convert the undistorted numpy array back to an Open3D image
    color_undistorted_o3d = o3d.geometry.Image(color_undistorted_np)
    depth_undistorted_o3d = o3d.geometry.Image(depth_undistorted_np)

    # --- Create the RGBD image with the undistorted color image ---
    rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(
        color_undistorted_o3d,  # Use the corrected color image
        depth_undistorted_np,
        depth_scale=1000.0,
        depth_trunc=5.0,
